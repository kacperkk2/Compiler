...
{
    std::string id;
    bool is_algo_attr; // czy jest definicja czy sama deklaracja
...
struct Print_node : Base_node
{
    ...
    Print_node() {node_type = PRINT_NODE; is_id_in_content = false;}
...
struct Condition_node : Base_node
{
    ...
    void to_string(std::string tabs)
    {
        ...
        logic->to_string(tabs + "\t");
        for (auto &els : if_statements)
            els->to_string(tabs+"\t");
        for (auto &ifs : else_statements)
            ifs->to_string(tabs+"\t");
        std::cout<<tabs+"}"<<std::endl;
    }
};
